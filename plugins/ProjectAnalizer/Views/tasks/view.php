        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                            ...
                            <div class="title-button-group">
                                <span class="dropdown inline-block">
                                    ...
                                    <ul class="dropdown-menu float-end" role="menu">
                                        <li role="presentation"><?php echo modal_anchor(get_uri("projectanalizer/task_modal_form"), "<i data-feather='edit-2' class='icon-16'></i> " . app_lang('edit_task'), array("title" => app_lang('edit_task'), "data-post-id" => $model_info->id, "data-post-view_type" => "details", "id" => "task-details-edit-btn", "class" => "dropdown-item")); ?></li>
                                        <li role="presentation"><?php echo modal_anchor(get_uri("projectanalizer/task_modal_form"), "<i data-feather='copy' class='icon-16'></i> " . app_lang('clone_task'), array("title" => app_lang('clone_task'), "data-post-id" => $model_info->id, "data-post-is_clone" => true, "data-post-view_type" => "details", "class" => "dropdown-item")); ?></li>
                                    </ul>
                                </span>
                            </div>
                        <?php } ?>
                    ...
                    <div class="card-body">
                        <?php echo view("ProjectAnalizer\\Views\\tasks\\task_view_data"); ?>
                    </div>
...
<?php } else { ?>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var browserState = {
                Url: "<?php echo get_uri("projectanalizer/task_view/" . $model_info->id); ?>"
            };
    ...
    <div class="modal-footer">
        <?php
        if ($can_edit_tasks) {
            echo modal_anchor(get_uri("projectanalizer/task_modal_form"), "<i data-feather='copy' class='icon-16'></i> " . app_lang('clone_task'), array("class" => "btn btn-default float-start", "data-post-is_clone" => true, "data-post-id" => $model_info->id, "title" => app_lang('clone_task')));
            echo modal_anchor(get_uri("projectanalizer/task_modal_form"), "<i data-feather='edit-2' class='icon-16'></i> " . app_lang('edit_task'), array("class" => "btn btn-default", "data-post-id" => $model_info->id, "title" => app_lang('edit_task')));
        }
        ?>
...
<?php
$task_link = anchor(get_uri("projectanalizer/task_view/$model_info->id"), '<i data-feather="external-link" class="icon-16 task-link-btn"></i>', array("target" => "_blank", "class" => "p15"));
?>
...
<script type="text/javascript">
    $(document).ready(function() {
        $("#task-percentage-save").on("click", function(e) {
            e.preventDefault();
            var taskId = $(this).data("task-id");
            var percentage = $("#task-percentage-input").val();
            var $error = $("#task-percentage-error");
            $error.addClass("hide").text("");

            appAjaxRequest({
                url: "<?php echo get_uri("projectanalizer/update_task_percentage"); ?>",
                type: "POST",
                dataType: "json",
                data: {
                    id: taskId,
                ...
                success: function(response) {
                    if (response && response.success) {
                        $("#task-percentage-input").val(response.percentage);
                    } else if (response && response.message) {
                        $error.removeClass("hide").text(response.message);
                    } else {
                        $error.removeClass("hide").text("Ocorreu um erro ao salvar.");
                    }
                }
            });
        });
