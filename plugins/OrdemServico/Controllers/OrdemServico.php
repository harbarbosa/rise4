...
class OrdemServico extends Security_Controller
{
    ...
    private function egestor_http_request(string $method, string $url, array $query = [], array $body = []): array
    {
        ...
        if ($response === false) {
            return [
                'status' => false,
                'code' => 500,
                'message' => $curl_error ?: 'Erro de conexão com o eGestor.',
            ];
        }
        ...
        return [
            'status' => false,
            'code' => $http_code ?: 500,
            'message' => 'Resposta inválida do eGestor.',
            'raw' => $response,
        ];
    }
    ...
    public function egestor_sync_os($codigo = 0)
    {
        ...
        if (!$codigo) {
            return $this->response->setJSON([
                'status' => false,
                'message' => 'Informe o código da OS.',
            ])->setStatusCode(422);
        }
        ...
        if (!$save_id) {
            return $this->response->setJSON([
                'status' => false,
                'message' => 'Não foi possível salvar a OS localmente.',
            ])->setStatusCode(500);
        }
