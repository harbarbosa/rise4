...
class Checklist_groups_model extends EngenhariaBaseModel {
    protected $table = 'eng_checklist_groups'; public function __construct(){ parent::__construct($this->table); }
    ...
    public function save_record(array $data, int $id = 0): int { $table=$this->db->prefixTable($this->table); if($id){$data['updated_at']=date('Y-m-d H:i:s');$this->db->table($table)->where('id',$id)->update($data);return $id;} $data+=array('sort'=>0,'is_active'=>1,'deleted'=>0,'created_at'=>date('Y-m-d H:i:s'),'updated_at'=>date('Y-m-d H:i:s'));$this->db->table($table)->insert($data);return (int)$this->db->insertID(); }
