...
class External_cache_model extends Crud_model
{
    protected $table = 'fv_external_cache';
    protected $primaryKey = 'id';
    protected $returnType = 'object';
    protected $useSoftDeletes = true;
    protected $deletedField = 'deleted';
    protected $allowedFields = array('provider', 'cache_key', 'endpoint', 'request_hash', 'payload_json', 'expires_at', 'created_by', 'created_at', 'updated_at', 'deleted');
    ...
    public function get_by_cache_key($cache_key = '')
    {
        ...
        try {
            ...
            log_message('error', '[Fotovoltaico] External cache query error: ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function put_cache($data = array())
    {
        ...
        $payload = array(
            ...
            'request_hash' => trim((string) get_array_value($data, 'request_hash')),
    ...
    public function invalidate_by_prefix($provider = '', $endpoint_prefix = '')
    {
        $table = $this->db->prefixTable($this->table);
        if (!$this->db->tableExists($table)) {
            return false;
        ...
        $endpoint_prefix = trim((string) $endpoint_prefix);
        if ($endpoint_prefix !== '') {
            $builder->like('endpoint', $endpoint_prefix, 'after');
        }
