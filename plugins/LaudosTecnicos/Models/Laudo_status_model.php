...
class Laudo_status_model extends Crud_model
{
    protected $table = null;

    public function __construct()
    {
        $this->table = 'laudo_status';
    ...
    public function get_details($options = array())
    {
        ...
        if ($status_filter !== null && $status_filter !== '') {
            $where .= " AND $table.active='" . ($status_filter ? '1' : '0') . "'";
        }
    ...
    {
        $table = $this->db->prefixTable($this->table);
        $sql = "SELECT id, name FROM $table WHERE deleted=0 AND active=1 ORDER BY sort_order ASC, name ASC";
        $result = $this->db->query($sql)->getResult();
        
        $dropdown = array();
        foreach ($result as $row) {
            $dropdown[$row->code] = $row->name;
        }
        return $dropdown;
    }
    ...
    {
        $table = $this->db->prefixTable($this->table);
        $sql = "SELECT * FROM $table WHERE deleted=0 AND active=1 ORDER BY sort_order ASC, name ASC";
