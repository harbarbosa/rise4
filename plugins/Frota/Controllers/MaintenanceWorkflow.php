...
class MaintenanceWorkflow extends Security_Controller
{
    ...
    protected function requireMaintenanceAccess(): void
    {
        ...
        if (get_array_value($permissions, 'frota_maintenance') != '1' && get_array_value($permissions, 'frota_manage') != '1') {
    ...
    public function issuesByVehicle($vehicleId)
    {
        ...
        $linkedIds = [];
        if ($maintenanceId) {
            $links = $this->table('frota_maintenance_issue_links')
                ->select('issue_id')
        ...
        $builder = $this->table('frota_issues')
        ...
        foreach ($rows as $row) {
            $data[] = [
                ...
                'selected' => in_array((int)$row['id'], $linkedIds, true)
            ];
        }
    ...
    public function save()
    {
        ...
        if (!is_array($issueIds)) {
            $issueIds = $issueIds ? explode(',', (string)$issueIds) : [];
        }
        ...
        try {
            ...
            foreach ($issueIds as $issueId) {
                $issue = $this->table('frota_issues')
                    ->where('id', $issueId)
                    ->where('vehicle_id', (int)($data['vehicle_id'] ?? 0))
                    ->where('deleted', 0)
                    ->get()->getRowArray();
                if (!$issue) {
                    continue;
                }
                $this->table('frota_maintenance_issue_links')->insert([
                    'maintenance_id' => $id,
                    'issue_id' => $issueId,
            ...
            if (!$this->db->transStatus()) {
                throw new \RuntimeException('Falha ao salvar manutenção.');
            }
        } catch (\Throwable $e) {
