...
<?php echo form_open(get_uri('purchases_suppliers/save'), array('id' => 'supplier-form', 'class' => 'general-form', 'role' => 'form')); ?>
<div class="modal-body clearfix">
    <input type="hidden" name="id" value="<?php echo esc($model_info->id); ?>" />

    <div class="form-group">
        <label for="name" class="form-label"><?php echo app_lang('purchases_supplier_name'); ?></label>
        <input id="name" name="name" value="<?php echo esc($model_info->name); ?>" class="form-control" placeholder="<?php echo app_lang('purchases_supplier_name'); ?>" />
    </div>

    <div class="form-group">
        <label for="email" class="form-label"><?php echo app_lang('purchases_supplier_email'); ?></label>
        <input id="email" name="email" value="<?php echo esc($model_info->email); ?>" class="form-control" />
    </div>

    <div class="form-group">
        <label for="phone" class="form-label"><?php echo app_lang('purchases_supplier_phone'); ?></label>
        <input id="phone" name="phone" value="<?php echo esc($model_info->phone); ?>" class="form-control" />
    </div>

    <div class="form-group">
        <label for="tax_id" class="form-label"><?php echo app_lang('purchases_supplier_tax_id'); ?></label>
        <input id="tax_id" name="tax_id" value="<?php echo esc($model_info->tax_id); ?>" class="form-control" />
    </div>

    <div class="form-group">
        <label for="address" class="form-label"><?php echo app_lang('purchases_supplier_address'); ?></label>
    ...
    $(document).ready(function () {
        $("#supplier-form").appForm({
            onSuccess: function (result) {
                $("#purchases-suppliers-table").appTable({newData: result.data, dataId: result.id});
            }
        });
    });
</script>
