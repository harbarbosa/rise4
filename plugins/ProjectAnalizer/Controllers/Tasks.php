...
class Tasks extends Security_Controller {
    ...
    public $project_member_memory = array(); //array([project_id]=>true/false)
    public $project_client_memory = array(); //array([project_id]=>true/false)
    public $can_edit_client_memory = array(); //array([client_id]=>true/false, [any_clients]=>true/false)
    public $can_access_lead_memory = array(); //array([client_id]=>true/false, [any_leads]=>true/false)
    public $can_access_estimate_memory = array(); //array([client_id]=>true/false, [any_estimates]=>true/false)
    public $can_edit_subscription_memory = array(); //array([client_id]=>true/false, [any_subscriptions]=>true/false)
    public $can_edit_ticket_memory = array(); //array([client_id]=>true/false, [any_tickets]=>true/false)
    public $projects_id_by_title = array();
    public $milestones_id_by_title = array();
    public $users_id_by_name = array();
    public $task_statuses_id_by_title = array();
    public $task_labels_id_by_title = array();
    public $task_priorities_id_by_title = array();
    ...
    public function get_context_and_id($model_info = null) {
    ...
    public function _client_can_create_tasks($context, $project_id) {
    ...
    public function _can_edit_clients($context_id) {
    ...
    public function _can_access_this_lead($context_id) {
    ...
    public function _can_access_this_estimate($context_id) {
    ...
    public function _can_edit_subscriptions($context_id) {
    ...
    public function _can_edit_tickets($context_id) {
    ...
    public function _is_clients_project($project_id) {
        //this method will be used a lot in loop. To reduce db call, save the value in memory. 
    ...
    public function _is_user_a_project_member($project_id) {
    ...
    public function _can_comment_on_tasks($task_info) {
    ...
    public function _can_edit_non_context_tasks($task_info) {
    ...
    public function _can_delete_non_context_tasks($task_info) {
        return $this->login_user->is_admin || $task_info->created_by === $this->login_user->id;
    }

    public function can_edit_tasks($_task = null) {
    ...
    public function _can_edit_task_status($task_info) {
    ...
    public function can_delete_tasks($_task = null) {
        $task_info = is_object($_task) ? $_task : $this->Tasks_model->get_one($_task); //the $_task is either task id or task info
        $permissions = $this->login_user->permissions;
    ...
    public function _prepare_query_parameters_for_accessible_contexts($contexts) {
    ...
    public function _send_task_updated_notification($task_info, $activity_log_id) {
    ...
    public function _send_task_created_notification($task_info) {
        if ($task_info->context === "project") {
    ...
    function save() {
        ...
        if ($start_date) {
            $start_date = $this->_check_valid_date($start_date);
            if ($start_date && $start_time) {
                $start_date = $start_date . " " . $start_time;
            }
        }
        if ($deadline) {
            $deadline = $this->_check_valid_date($deadline);
    ...
    public function _make_row($data, $custom_fields, $show_time_with_task, $tasks_edit_permissions, $tasks_status_edit_permissions, $is_mobile = 0) {
    ...
    public function _get_collaborators($collaborator_list, $clickable = true) {
        $collaborators = "";
    ...
    public function check_sub_tasks_statuses($status_id = 0, $parent_task_id = 0) {
        if ($status_id !== "3") {
    ...
    public function _make_checklist_item_row($data, $return_type = "row") {
        $checkbox_class = "checkbox-blank";
        $title_class = "";
        $is_checked_value = 1;
    ...
    public function _make_sub_task_row($data, $return_type = "row") {
    ...
    public function _make_dependency_tasks_view_data($task_ids = "", $task_id = 0, $type = "") {
        if ($task_ids) {
            $tasks = "";
    ...
    public function _make_dependency_tasks_row_data($task_info, $task_id, $type) {
        $tasks = "";
    ...
    public function _get_all_dependency_for_this_task_specific($task_ids = "", $task_id = 0, $type = "") {
        if ($task_id && $type) {
    ...
    function all_gantt() {
        ...
        return $this->template->rander("ProjectAnalizer\\Views\\projects\\gantt\\index", $view_data);
    }
    ...
    public function _get_all_dependency_for_this_task($task_id) {
        $task_info = $this->Tasks_model->get_one($task_id);
    ...
    public function _get_collaborators_ids($collaborators_data) {
    ...
    public function _get_only_own_projects_user_id() {
    ...
    public function _get_tasks_edit_permissions($tasks = array()) {
    ...
    public function _get_tasks_status_edit_permissions($tasks = array(), $tasks_edit_permissions = array()) {
        $permissions = array();
        foreach ($tasks as $task_info) {
        ...
        if ($project_id) {
            ...
            foreach ($statuses as $status) {
                $status_dropdown[] = array(
                    "id" => $status->id,
                    "value" => $status->id,
                    "text" => ($status->key_name ? app_lang($status->key_name) : $status->title),
                    "isSelected" => true
                );
            }
            ...
            return $this->template->view("ProjectAnalizer\\Views\\projects\\gantt\\index", $view_data);
        }
    }

    function evolution_project($project_id = 0) {
        return $this->gantt($project_id);
    }
    ...
    function gantt_chart_view($project_id = 0) {
        ...
        // Mostrar todos os status por padrão no Gantt.
    ...
    public function invalid_date_of_gantt($start_date, $end_date) {
    ...
    public function _validate_excel_import_access() {
        return $this->can_create_tasks("project");
    }

    public function _get_controller_slag() {
        return "tasks";
    }

    public function _get_custom_field_context() {
        return "tasks";
    }

    public function _get_headers_for_import() {
    ...
    public function _init_required_data_before_starting_import() {
    ...
    public function _save_a_row_of_excel_data($row_data) {
        $now = get_current_utc_time();
    ...
    public function _prepare_task_data($row_data) {
    ...
    public function _prepare_common_gantt_filters($project_id = 0) {
        $view_data['milestone_dropdown'] = $this->_get_milestones_dropdown_list($project_id);
        $view_data["show_milestone_info"] = $this->can_view_milestones();
